...
with st.sidebar:
    ...
    #  st.image("./adorable_cat.png", use_column_width=True) lokal
     ...
     try:
        ...
        # In case of an error, set to min_date and max_date
        start_date = min_date
        end_date = max_date
...
colors_bestprod = ["#FFA62F", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"]
colors_worstprod = ["#FF4545", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"]

sns.barplot(x="product_qty", y="product_category_name_english", data=sum_order_items_df.head(), palette=colors_bestprod, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel(None)
ax[0].set_title("Best Performing Product", loc="center", fontsize=15)
ax[0].tick_params(axis ='y', labelsize=12)

sns.barplot(x="product_qty", y="product_category_name_english", data=sum_order_items_df.sort_values(by="product_qty", ascending=True).head(), palette=colors_worstprod, ax=ax[1])
